...
if(keyDown("DOWN_ARROW") && player.y<height-70||touches.length>0){
 player.y = player.y+30
}
...
if(gameState == "lost"){
  ...
  textSize(100)
  fill("red")
  text("You Lost ",width/2-150,height/2+40)
...
//destroy zombie and player and display a message in gameState "won"
  ...
  textSize(100)
  fill("yellow")
  text("You Won ",width/2-180,height/2+40)
  zombieGroup.destroyEach();
  player.destroy();
...
//destroy zombie, player and bullets and display a message in gameState "bullet"
  ...
  textSize(50)
  fill("red")
  text("You ran out of bullets!!!",width/2-180,height-200)
  zombieGroup.destroyEach();
  player.destroy();
  bulletGroup.destroyEach();
...
function enemy(){
  if(frameCount%50===0){
    ...
    zombie = createSprite(random(width/2,width-120),random(height/2,height-120),40,40)
